...
/**
 * Room object that contains the legal paths through itself.
 */
var Room = (function() {
  /**
   * @param type [0-13]
...
function moveThroughMaze( indy, tryOtherWay ) {
  ...
  if (tryOtherWay) {
    ...
    if (invalidExit( nextExit, indy.x )) {   // Then try right
      newRoom.unrotate();  // undo the left first
      newRoom.rotate( RIGHT );
      command = indy.x + " " + indy.y + " " + RIGHT;

      nextExit = newRoom.getExitForEntrance( newEntranceDir );
      if (invalidExit( nextExit, indy.x )) {  // try double right (reverse)
...
function readRocks( outRocks ) {
  var numRocks = parseInt(readline());
  for (i = 0; i < numRocks; i++) {
    inputs = readline().split(' ');
    outRocks[i] = outRocks[i] || {};
    outRocks[i].x = parseInt(inputs[0]);
    outRocks[i].y = parseInt(inputs[1]);
    outRocks[i].enteringFrom =  inputs[2];
  }
}
